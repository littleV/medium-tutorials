...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        _content.image = [UIImage imageNamed:@"1"];
...
- (void) pageTurn: (UIPageControl *) page
{
    ...
    if (i == 0) {
        ...
        _content.image = [UIImage imageNamed:@"2"];
        _content.center = page.center;
    } else if (i == 2) {
        _content.image = [UIImage imageNamed:@"3"];
        _content.center = page.center;
    }
    [self updateDots];
}
...
- (void) updateDots
{
    ...
    for (int i = 0; i < self.numberOfPages; i ++) {
        ...
        if (i == self.currentPage) {
            ...
            status.layer.cornerRadius = status.frame.size.height/2;
            status.layer.borderColor = [[UIColor orangeColor] CGColor];
